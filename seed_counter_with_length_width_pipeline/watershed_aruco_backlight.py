...
localMaxtest = peak_local_max(D,  min_distance=15, labels=thresh)
...
for label in np.unique(labels):
    ...
    if cv2.contourArea(c) > dust_thresh:
        ...
        #cv2.putText(image, "w={}mm\u00b2, h={}mm\u00b2".format(w,h), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 10)
        
        
        w=round(rect[1][1] * 12.7  / average_length,2)
        h=round(rect[1][0] * 12.7 / average_length,2)
        cv2.putText(image, "w={}, h={}".format(w,h), (int(rect[0][0]),int(rect[0][1])),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
    if cv2.contourArea(c) < dust_thresh:
...
area
square_irl = 12.7**2.0
temp_mm=area * square_irl / np.average(aruco_areas[0:4])
#average length and widths of aruco markers 8 numbers
#average_length=np.average(aruco_lenth+aruco_width)
real_length = np.array(length) * 12.7 / average_length
real_width = np.array(width) * 12.7 / average_length

final=np.column_stack((area,temp_mm,real_length,real_width))




colnames = ['pixel_area','mm\u00b2','length','width']
df=pd.DataFrame(final,columns=colnames)



print("average size of objects is {} mm\u00b2" .format(np.average(temp_mm)))
#np.savetxt("area_mm.csv", df, delimiter=",", fmt='%f')
df.to_csv("area_mm.csv", index=False)
